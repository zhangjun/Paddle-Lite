...
kernel void FUNC3_(conv_add_1x1, PRELU_TYPE, P)(
  ...
  for (uint i = 0; i < input_arr_size; ++i) {
    ...
    VECTOR(P, 4)
    weight_x = weights[weithTo + 0 * kernelHXW * input_arr_size + i];
    output.x += dot(input, weight_x);

    VECTOR(P, 4)
    weight_y = weights[weithTo + 1 * kernelHXW * input_arr_size + i];
    output.y += dot(input, weight_y);

    VECTOR(P, 4)
    weight_z = weights[weithTo + 2 * kernelHXW * input_arr_size + i];
    output.z += dot(input, weight_z);

    VECTOR(P, 4)
    weight_w = weights[weithTo + 3 * kernelHXW * input_arr_size + i];
...
kernel void FUNC3_(conv_add_3x3, PRELU_TYPE, P)(
  ...
  for (uint i = 0; i < input_arr_size; ++i) {
    ...
    for (int j = 0; j < 9; ++j) {
      VECTOR(P, 4)
      weight_x = weights[weithTo + 0 * kernelHXW * input_arr_size +
                         j * input_arr_size + i];
      output.x += dot(input[j], weight_x);

      VECTOR(P, 4)
      weight_y = weights[weithTo + 1 * kernelHXW * input_arr_size +
                         j * input_arr_size + i];
      output.y += dot(input[j], weight_y);

      VECTOR(P, 4)
      weight_z = weights[weithTo + 2 * kernelHXW * input_arr_size +
                         j * input_arr_size + i];
      output.z += dot(input[j], weight_z);

      VECTOR(P, 4)
      weight_w = weights[weithTo + 3 * kernelHXW * input_arr_size +
                         j * input_arr_size + i];
