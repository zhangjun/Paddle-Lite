...
kernel void FUNC(softmax_c_d3_common,
  ...
  VECTOR(P, 4)
  temp_value_vector = inTexture.read(uint2(gid.x, gid.y), array_size - 1);
  ...
  VECTOR(P, 4)
  temp_value_vector_left = inTexture.read(uint2(gid.x, gid.y), array_size - 1);
...
kernel void FUNC(softmax_w_2d_common,
  ...
  VECTOR(P, 4) sum_vector_left = 0.0;
  VECTOR(P, 4)
  temp_value_vector_left = inTexture.read(uint2((sp.W + 3) / 4, gid.y), 0);
  sum_vector_left += exp(temp_value_vector_left - max_value);
